...
CONFIG="${DESTV}/grabConfig.txt"

CAM_0="${VID_DIR}/Cam0"
[ -d ${CAM_0} ] || mkdir -p "${CAM_0}"
DEST="${CAM_0}/Popa${TIMESTAMP}0.mp4"
echo "$DEST"
#rpicam-vid -v --camera 0 -t 5s --config "${CONFIG}" -o "${DEST}"
rpicam-vid --camera 0 -t 15s --config "${CONFIG}" -o "${DEST}" >"${LOG}" 2>&1

CAM_1="${VID_DIR}/Cam1"
[ -d ${CAM_1} ] || mkdir -p "${CAM_1}"
DEST="${CAM_1}/Popa${TIMESTAMP}1.mp4"
echo "$DEST"
rpicam-vid --camera 1 -t 15s --config "${CONFIG}" -o "${DEST}" >>"${LOG}" 2>&1
